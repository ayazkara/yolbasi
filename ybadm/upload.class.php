...
class upload{
    ...
    public function save($file=NULL){
        ...
        if($file==NULL){
            die('class save(\'\') file deðeri geçersiz');
        }else{
            if($_FILES[$file]['error']==0 && $_FILES[$file]['size']!=0){
                if(!$this->check_allowed_ext($_FILES[$file])){
                    echo '<script>window.alert("Geçersiz Dosya Uzantýsý")</script>';
                }elseif(!$this->check_allowed_size($_FILES[$file])){
                    $this->error[] = array(2,'Upload limit aþýmý. Maksimum upload boyutu '.$this->allowed_size);
                }elseif(!$this->check_image($_FILES[$file])){
                    $this->error[] = array(3,'Geçersiz resim dosyasý');
                }else{
                    if(move_uploaded_file($_FILES[$file]['tmp_name'], $this->directory . $this->path($_FILES[$file]))){
                        ...
                        $this->error[] = array(0,'Upload sýrasýnda hata oluþtu');
                        return false;
                    }
                }
            }else{
